    ...
    this->window = Window(GTK_WINDOW_TOPLEVEL, false);
        this->contents = Panel(false, 0);
            ...
            Label header = Label("Log");
                ...
                contents.Pack_start(header.GetWidget(), true, true, 0);
            ...
            Panel logInfoPanel = Panel(true, 5);
                ...
                this->logRecordedTime = Label(newTime);
                    logInfoPanel.Pack_start(logRecordedTime.GetWidget(), true, true, 0);

                contents.Pack_start(logInfoPanel.GetWidget(), true, true, 10);
            ...
            createHorizontalReadout("Running Time: ", this->totalRunningTime);

            //total frames readout
            this->totalFrames = Label(std::to_string(totalFramesNum));
            createHorizontalReadout("Total Frames: ", totalFrames);
            ...
            createHorizontalReadout("Frames With Target Seen: ", framesWithTargetSeen);

            //target lost event count readout
            this->targetLostEventCount = Label(std::to_string(targetLostEventCountNum));
            createHorizontalReadout("Number of times the target was lost: ", targetLostEventCount);

            //average frame time readout
            int avgFPS = (int) (1000 / averageFrameTimeNum);
            std::string avgFrameTimeString = std::to_string(averageFrameTimeNum) + " (" + std::to_string(avgFPS) + " FPS)";
            this->averageFrameTime = Label(avgFrameTimeString);
            createHorizontalReadout("Average Frame Time: ", averageFrameTime);

            //fastest frame time readout
            int fastestFPS = 1000 / fastestFrameTimeNum;
            std::string fastestFrameTimeString = std::to_string(fastestFrameTimeNum) + " (" + std::to_string(fastestFPS) + " FPS)";
            this->fastestFrameTime = Label(fastestFrameTimeString);
            createHorizontalReadout("Fastest Frame Time: ", fastestFrameTime);

            //slowest frame time readout
            int slowestFPS = 1000 / slowestFrameTimeNum;
            std::string slowestFrameTimeString = std::to_string(slowestFrameTimeNum) + " (" + std::to_string(slowestFPS) + " FPS)";
            this->slowestFrameTime = Label(slowestFrameTimeString);
            createHorizontalReadout("Slowest Frame Time: ", slowestFrameTime);

            //average distance readout
            this->averageDistance = Label(std::to_string(averageDistanceNum));
            createHorizontalReadout("Average Distance: ", averageDistance);

            //closest distance readout
            this->closestDistance = Label(std::to_string(closestDistanceNum));
            createHorizontalReadout("Closest Distance: ", closestDistance);

            //farthest distance readout
            this->farthestDistance = Label(std::to_string(farthestDistanceNum));
            createHorizontalReadout("Farthest Distance: ", farthestDistance);
...
void LogViewer::createHorizontalReadout(std::string header, Label readout) {
    Panel readoutPanel = Panel(true, 0);
        Label headerLabel = Label(header);
            headerLabel.SetName("gray");
            readoutPanel.Pack_start(headerLabel.GetWidget(), false, false, 0);
            readoutPanel.Pack_start(readout.GetWidget(), false, false, 0);
        contents.Pack_start(readoutPanel.GetWidget(), false, false, 0);
}
